...
def test_autoload_classifies_and_skips_repeats(conn, tmp_path):
    ...
    # настоящая форма 5-ТН содержит десятки строк; распознавание требует,
    # чтобы коды стояли в одной графе и их было не меньше пяти
    _form_xlsx(tmp_path / "5tn_2019_50.xlsx",
               [["НП ЮЛ", "1100", 10], ["НП ФЛ", "2100", 90],
                ["ТС в базе ЮЛ", "1200", 15], ["ТС в базе ФЛ", "2200", 85],
                ["ТС начислен ЮЛ", "1300", 14], ["ТС начислен ФЛ", "2300", 80],
                ["Сумма ЮЛ", "1400", 50], ["Сумма ФЛ", "2400", 70],
                ["Льготы ЮЛ", "1500", 5]],
               "Отчёт по форме 5-ТН")
